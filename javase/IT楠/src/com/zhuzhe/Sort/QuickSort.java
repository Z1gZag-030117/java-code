    ...
    private static int get_mid(int arr[], int left, int right) {
        ...
        while (left < right) {//当left与right指针相遇的时候退出循环，双指针遍历结束
            while (arr[right] >= pivot && left < right) {
                right--;
            }//right指针从右往左遍历，当arr[right]>=pivot，即满足以pivot为中轴，小放左，大放右的条件时，right指针继续往右遍历。当arr[right]<pivotd的时候，把当前值arr[right]赋给空置arr[left]，此时arr[right]成了空值。
            arr[left] = arr[right];
            while (arr[left] <= pivot && left < right) {
                left++;
            }//到left指针从左往右遍历，当arr[left]<=pivot，即满足以pivot为中轴，小放左，大放右的条件时，left指针继续往左遍历。当arr[left]>pivot的时候，把当前值arr[left]赋给空置arr[right]，此时arr[left]成了空值。
            arr[right] = arr[left];
        }
